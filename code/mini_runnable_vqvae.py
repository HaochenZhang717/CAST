...
def plot_neighbor_time_series(
    ...
    """
    Plot anchor + nearest neighbors in ONE figure (multiple lines).
    """
    ...
    for row, emb_idx in enumerate(all_idxs):
        ...
        if row == 0:
            # anchor
            plt.plot(signal, lw=2.5, label="Anchor", zorder=10)
        else:
            plt.plot(signal, lw=1.0, alpha=0.6)

    plt.title("Anchor and Nearest Neighbors (Latent Space)", fontsize=12)
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig("neighbor.png", dpi=200)
    plt.close()
...
def plot_random_time_series(
    ...
    """
    Plot anchor + RANDOM samples in ONE figure (multiple lines).
    """
    all_idxs = [anchor_emb_idx] + neighbor_emb_indices.tolist()
    plt.figure(figsize=(3, 2))
    ...
    plt.title("Anchor and Random Samples", fontsize=12)
    plt.grid(alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
def trim_padding(signal, eps=1e-6):
    """
    signal: np.array, shape (T,)
    returns: trimmed signal up to last non-zero (or > eps) entry
    """
    if signal.ndim != 1:
        raise ValueError("signal must be 1D")

    valid = np.abs(signal) > eps
    if not np.any(valid):
        return signal[:1]  # 极端情况：全是 0

    last_idx = np.where(valid)[0][-1]
    return signal[: last_idx + 1]

def plot_nn_vs_random(
    code_segments,
    valid_indices,
    anchor_emb_idx,
    nn_indices,
    ...
    # ---------- Anchor ----------
    anchor_seg_idx = valid_indices[anchor_emb_idx]
    anchor_signal = code_segments[anchor_seg_idx][signal_key][:, 0]
    if isinstance(anchor_signal, torch.Tensor):
        anchor_signal = anchor_signal.cpu().numpy()

    anchor_signal = trim_padding(anchor_signal)

    if max_len is not None:
        anchor_signal = anchor_signal[:max_len]
    ...
    # ---------- Nearest Neighbors ----------
    for emb_idx in nn_indices:
        seg_idx = valid_indices[emb_idx]
        sig = code_segments[seg_idx][signal_key][:, 0]
        if isinstance(sig, torch.Tensor):
            sig = sig.cpu().numpy()

        sig = trim_padding(sig)
    ...
    # ---------- Styling ----------
    plt.title("Latent Neighbors vs. Random Samples", fontsize=8)
    plt.xlabel("Time Steps", fontsize=7)
    plt.ylabel("Normalized Value", fontsize=7)
    ...
    plt.legend(
        handles=[
            # plt.Line2D([], [], color="#1f77b4", lw=2.5, label="Anchor"),
            plt.Line2D([], [], color="#4C72B0", lw=1.2, label="Latent Neighbors"),
            plt.Line2D([], [], color="#C44E52", lw=1.2, label="Random Selected"),
        ],
        fontsize=7,
        frameon=False,
        loc="lower right",
    ...
    plt.tight_layout()
    plt.savefig(save_path, dpi=200)  # ❌ 不用 bbox_inches="tight"
    plt.close()
...
def inspect_embedding_neighborhood(
    ...
    random_indices = torch.randperm(len(valid_indices))[:len(neighbors)]
